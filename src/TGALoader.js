...
TGALoader.prototype = Object.assign(Object.create(Loader.prototype), {
  ...
  load: function (url, onLoad, onProgress, onError) {
    ...
    loader.load(
      url,
      function (buffer) {
        const { canvas, pixel_size } = scope.parse(buffer);
        texture.image = canvas;
        texture.needsUpdate = true;
        const channels = parseInt(
          buffer.byteLength / (canvas.width * canvas.height)
        );
        const bitDepth = pixel_size / channels;
  ...
  parse: function (buffer) {
    ...
    return { canvas, pixel_size: header.pixel_size };
  },
});
